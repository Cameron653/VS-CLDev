...
/datum/controller/subsystem/xenoarch/proc/SetupXenoarch()
	for(var/turf/simulated/mineral/M in world) //This selects every mineral turf in the world
		if(!M.density || (M.z in using_map.xenoarch_exempt_levels)) //Checks to see if it's a mineral wall and if it's on a non-exempt Z level.
			continue

		if(isnull(M.geologic_data))
			M.geologic_data = new /datum/geosample(M) //Gives turf a random 'fluff' description which can be examined via a core sampler.

		if(!prob(XENOARCH_SPAWN_CHANCE)) //Now we roll the dice. Base chance is 1/200 for a mineral turf to spawn a digsite. If it doesn't roll that chance, we skip this rock.
			continue
...
//Here, we start to see how many artifacts will spawn in the selected rock. 1-3 artifacts per.
			...
			if(isnull(archeo_turf.finds))
				...
				if(prob(50))
					...
					archeo_turf.finds.Add(new /datum/find(digsite, rand(10, 50))) 	//High chance of being visible, alerting xenoarch to a digsite location.
